...
NS_LOG_COMPONENT_DEFINE ("FirstProgram");
...
main (int argc, char *argv[])
{
  ...
  pointToPoint.EnablePcapAll ("fprogram");
  pointToPoint.EnablePcap ("fprogram", devices.Get (1), true);

  ApplicationContainer clientApps = udpClient.Install (nodes.Get (0));
  clientApps.Start (Seconds (2.0));
  clientApps.Stop (Seconds (10.0));
  AnimationInterface anim("fprogram.xml");
